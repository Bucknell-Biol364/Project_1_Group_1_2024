...
1. The data set has been obtained by random sammples from a population
1. The variability of the data in each group is similar
...
The example t.test returned a p-value of 0.7936. The p-vlaue tells us the likelihood that you might observe the modeled outcome in your data set given that the null hypothesis is true. We will reject the null hypothesis if p < 0.05. Given our test returned a p-value of 0.7936, we can not confidently say amongst clay or sandy soil that soil type affects the length of sepals in *I. setosa* so we fail to reject the null hypothesis.
